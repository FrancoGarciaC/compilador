...
elabTerm ((Decl pos name body):xs) = 
  if null xs then body 
  else let letBody = elabTerm' xs in
       Let (getInfo body) name (getInfo letBody) body (close name letBody)
                                   
  where                                   
    elabTerm' [Decl _ _ body] = replaceGlobal body
    elabTerm' ((Decl pos name body):xs) = let letBody =  close name (elabTerm' xs) in
                                          Let (getInfo body) name NatTy (replaceGlobal body) letBody
...
data Closure = ClosFun Env Name Ty TTerm | ClosFix Env Name Ty Name Ty TTerm  deriving Show
